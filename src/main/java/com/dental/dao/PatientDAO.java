...
public class PatientDAO {
    ...
    public void addPatient(Patient p) throws Exception {
        String sql = "INSERT INTO Patients (FirstName, LastName, Email, Phone, Address, DateOfBirth, MedicalHistory, Allergies) VALUES (?,?,?,?,?,?,?,?)";
    ...
    public void updatePatient(Patient p) throws Exception {
        String sql = "UPDATE Patients SET FirstName=?, LastName=?, Email=?, Phone=?, Address=?, DateOfBirth=?, MedicalHistory=?, Allergies=?, UpdatedAt=SYSUTCDATETIME() WHERE PatientID=?";
        try (Connection conn = DBUtil.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, p.getFirstName());
            ps.setString(2, p.getLastName());
